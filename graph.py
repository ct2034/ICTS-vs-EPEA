...
def save_graphs():
    ...
    save_runtime_graph(epea_results, icts_results,
                       maze_runtime_labels, 'maze_runtime_medians.pdf')

    maze_expansion_epea_labels = get_node_expansion_labels('EPEA*', 'Maze')
    save_expansion_graph(epea_results, maze_expansion_epea_labels,
                         'maze_epea_expansions.pdf', 'EPEA')

    maze_expansion_icts_labels = get_node_expansion_labels('ICTS', 'Maze')
    save_expansion_graph(icts_results, maze_expansion_icts_labels,
                         'maze_icts_expansions.pdf', 'ICTS')

    maze_ratio_labels = get_ratio_labels('Maze')
    save_ratio_graph(epea_results, icts_results,
                     maze_ratio_labels, 'maze_ratio_means.pdf')

    maze_open_list_labels = get_open_list_labels('EPEA*', 'Maze')
    save_open_list_graph(epea_results, maze_open_list_labels,
                         'maze_epea_open_list.pdf', 'EPEA')

    maze_open_list_labels = get_open_list_labels('ICTS', 'Maze')
    save_open_list_graph(icts_results, maze_open_list_labels,
                         'maze_icts_open_list.pdf', 'ICTS')

    maze_joint_mdd_labels = get_joint_mdd_labels('Maze', 'Mean')
    save_joint_mdd_graph_means(
        icts_results, maze_joint_mdd_labels, 'maze_icts_joint_mdd_means.pdf', 'ICTS')
    maze_joint_mdd_labels = get_joint_mdd_labels('Maze', 'Median')
    save_joint_mdd_graph_medians(
        icts_results, maze_joint_mdd_labels, 'maze_icts_joint_mdd_medians.pdf', 'ICTS')
    ...
    save_runtime_graph(epea_open_results, icts_open_results,
                       open_runtime_labels, 'open_runtime_medians.pdf')

    open_expansion_epea_labels = get_node_expansion_labels('EPEA*', 'Open')
    save_expansion_graph(
        epea_open_results, open_expansion_epea_labels, 'open_epea_expansions.pdf', 'EPEA')

    open_expansion_icts_labels = get_node_expansion_labels('ICTS', 'Open')
    save_expansion_graph(
        icts_open_results, open_expansion_icts_labels, 'open_icts_expansions.pdf', 'ICTS')

    open_ratio_labels = get_ratio_labels('Open')
    save_ratio_graph(epea_open_results, icts_open_results,
                     open_ratio_labels, 'open_ratio_means.pdf')

    open_open_list_labels = get_open_list_labels('EPEA*', 'Open')
    save_open_list_graph(
        epea_open_results, open_open_list_labels, 'open_epea_open_list.pdf', 'EPEA')

    open_open_list_labels = get_open_list_labels('ICTS', 'Open')
    save_open_list_graph(
        icts_open_results, open_open_list_labels, 'open_icts_open_list.pdf', 'ICTS')

    open_joint_mdd_labels = get_joint_mdd_labels('Open', 'Mean')
    save_joint_mdd_graph_means(
        icts_open_results, open_joint_mdd_labels, 'open_icts_joint_mdd_means.pdf', 'Open')
    open_joint_mdd_labels = get_joint_mdd_labels('Open', 'Median')
    save_joint_mdd_graph_medians(
        icts_open_results, open_joint_mdd_labels, 'open_icts_joint_mdd_medians.pdf', 'Open')


def get_file_names(algorithm, prefix):
    batch_prefix_names = [prefix + '12x12', prefix +
                          '25x25', prefix + '50x50', prefix + '100x100']
    batch_names = [batch + '_3_' + algorithm +
                   '_results.txt' for batch in batch_prefix_names]
    return batch_names
...
def save_runtime_graph(epea_results, icts_results, labels, output_file):
    ...
    y_lim = [-0.2, 3.5]
    save_beautiful_graph_medians(
        epea_median_times, icts_median_times, labels, output_file, y_lim)
...
def save_runtime_graph_means(epea_results, icts_results, labels, output_file):
    ...
    save_beautiful_graph_means(
        epea_means, epea_stddevs, icts_means, icts_stddevs, labels, output_file, [])
...
def save_expansion_graph(results, labels, output_file, algorithm):
    ...
    if algorithm == 'ICTS':
        ...
        y_lim = [-0.5, 370]
    save_beautiful_graph_medians(
        median_expansions, [], labels, output_file, y_lim, algorithm=algorithm)
...
def save_ratio_graph(epea_results, icts_results, labels, output_file):
    ...
    y_lim = [-0.1, 1.0]
    save_beautiful_graph_means(
        epea_means, epea_stddevs, icts_means, icts_stddevs, labels, output_file, y_lim)
...
def save_joint_mdd_graph_means(results, labels, output_file, algorithm):
    mean_expansions, stddev_expansions = get_means_and_stddevs(
        results, 'max_joint_mdd_visited')
    y_lim = []
    save_beautiful_graph_means(mean_expansions, stddev_expansions, [], [
    ], labels, output_file, y_lim, algorithm=algorithm)
...
def save_beautiful_graph_means(first_means, first_stddevs, second_means, second_stddevs, labels, filename, y_lim, algorithm=None):
    ...
    ax.fill_between(ticks, first_plus, first_minus,
                    alpha=0.25, facecolor=colors[0])
    ...
    if second_means != []:
        ...
        ax.fill_between(ticks, second_plus, second_minus,
                        alpha=0.25, facecolor=colors[4])
